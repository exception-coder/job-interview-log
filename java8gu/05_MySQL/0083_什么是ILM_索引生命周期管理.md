...
1. 从此，`my-logs-000001`进入只读状态，并开始计算它在生命周期中的年龄。
2. 当它存活了 1 天后，自动进入 Warm 阶段，执行迁移到暖节点和设为只读的动作。
3. 存活 7 天后，进入 Cold 阶段，执行迁移到冷节点的动作。
4. 存活 180 天后，进入冻结阶段，数据迁移到冻结节点。
5. 存活 1年后，进入 Delete 阶段，索引被自动删除。
