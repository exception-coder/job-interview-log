...
# 典型回答volatile通常被比喻成”轻量级的synchronized“，也是Java并发编程中比较重要的一个关键字。和synchronized不同，volatile是一个变量修饰符，只能用来修饰变量。无法修饰方法及代码块等。
​
...
i++被拆分成3个指令：
- 执行GETFIELD拿到主内存中的原始值number。- 执行IADD进行加1操作。- 执行PUTFIELD把工作内存中的值写回主内存中。- ​当多个线程并发执行PUTFIELD指令的时候，会出现写回主内存覆盖问题，所以才会导致最终结果不为 10000，所以 volatile 不能保证原子性。
​
