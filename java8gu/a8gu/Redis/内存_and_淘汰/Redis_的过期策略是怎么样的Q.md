...
- 如果已过期，Redis会立即删除这个键，并返回 `nil` 给客户端（就像这个键不存在一样）。- 如果未过期，则正常返回键的值。​
