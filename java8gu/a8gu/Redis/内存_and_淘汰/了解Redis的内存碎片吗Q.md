...
- **used_memory**：表示Redis为了保存数据实际申请使用的内存空间。- **used_memory_rss**：表示操作系统实际分配给Redis的物理内存空间，其中包含了内存空间碎片。- **mem_fragmentation_ratio**：表示Redis当前的内存碎片率。等于used_memory_rss/used_memory- **​**大于等于1但小于等于1.5，这种情况是合理的。- 大于1.5，表明内存碎片率已经超过了50%。说明碎片比较多了。​
