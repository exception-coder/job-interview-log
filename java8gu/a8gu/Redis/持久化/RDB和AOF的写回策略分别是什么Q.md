...
## RDB的写回策略在 Redis 中，RDB 的写回策略主要包括以下几个方面：
...
**策略**：
- Redis会定期检查这些条件，如果满足，触发 RDB 的保存操作。- 条件可以通过修改 `redis.conf` 文件自定义，也可以通过命令动态设置，例如：

#### 手动触发在Redis中，我们可以通过以下命令手动生成 RDB 文件：
​

- **SAVE**：会阻塞 Redis 服务器，直到快照完成。- **BGSAVE**：在后台异步生成 RDB 文件，不会阻塞 Redis。
