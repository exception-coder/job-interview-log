...
- 如果 (a - b > 0)，则 (a > b)。- 如果 (b - a > 0)，则 (b > a)。- 如果 (a - b = 0)，则 (a = b)。
...
- 如果 (a + b > 2a)，则 (b > a)。- 如果 (a + b > 2b)，则 (a > b)。- 如果 (a + b = 2a)，则 (b = a)。
...
- 计算差值 `diff = a - b`。- 通过差值的符号位来判断：- 如果 `diff` 为正，则 `a > b`。- 如果 `diff` 为负，则 `b > a`。- 如果 `diff` 为零，则 `a == b`。​

- `diff >> 31` 右移 31 位，将差值的符号位移到最低位（最右边），这样可以得到符号位。- `& 1` 用来提取符号位。符号位为 0 表示 `diff` 为非负（即 `a >= b`），为 1 表示 `diff` 为负（即 `a < b`）。- 如果 `diff` 为 0，说明 `a` 和 `b` 相等。
...
（这个方式挺逗比的，是我看到一个面经中面试官说可用这种方式的，这就很难评。。。。。。不建议大家提这个方案，如果面试官一定要刨根问底的追问更多方案，可以提一下。）
​
