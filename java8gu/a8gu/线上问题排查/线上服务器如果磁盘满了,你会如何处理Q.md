...
- **定期清理脚本**- 写 cron job 自动清理老日志

- **磁盘监控告警**- 使用 `Prometheus + Grafana` 或 `Zabbix` 监控磁盘使用率- 设置告警阈值（比如 80% 告警）

- **日志分级+限量**- 按需写日志（只写 ERROR 级别）- 配置日志轮转（logrotate）

- **考虑分离日志、数据盘**- 应用数据、日志单独挂载到不同分区，避免挤爆系统盘
