...
> MAC地址是与设备硬件直接关联的唯一标识符。通过获取到同一个 MAC 地址生成的大量UUID，可以被恶意用户或第三方通过反向工程解析出MAC地址，进而获取到设备的物理位置或用户身份信息。在某些情况下，如果MAC地址被泄露，它可能被用于针对特定设备的网络攻击。​
...
一个 UUIDv7 是一个 128 位的数字（32 个十六进制字符，以 `xxxxxxxx-xxxx-Mxxx-Nxxx-xxxxxxxxxxxx` 的形式显示）。它的结构经过精确定义，以嵌入一个时间戳。
| 字段名 | 位长 | 内容 |
| `unix_ts_ms` | 48 位 | 从 Unix 纪元（1970-01-01）至今的毫秒数。 |
| `ver` (版本) | 4 位 | 固定为数字 `7`（二进制 `0111`），用于标识这是 UUIDv7。 |
| `rand_a` | 12 位 | 加密级别的随机数。 |
| `Var` (变体) | 2 位 | UUID 变体位（标准规定为 `10`）。 |
| `rand_b` | 62 位 | 加密级别的随机数。 |
​

**UUIDv7 一定能保证有序吗？**
- **在宏观时间尺度上（跨毫秒），答案是肯定的。**- **在微观时间尺度上（同一毫秒内），答案取决于实现方式，但几乎所有主流实现都确保了其可排序性。** 使用计数器实现的库提供了绝对的顺序保证。​
