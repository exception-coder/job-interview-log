...
### filtered的意义**针对使用索引的查询来说，**`**filtered**`** 字段表示查询过程中通过索引扫描的行数中（其实就是执行计划中的rows字段），经过**`**WHERE**`** 条件过滤后，预计会保留下来的行的百分比（是个预估值）。这个值介于0.00-100.00之间。**
​
...
- rows表示使用a这个索引扫描的行数。比如是10000- filtered表示使用b作为查询条件过滤后保留下来的数据的百分比。比如是10.00- 那么最终就会有10000*10% = 1000条数据被保留下来。​
