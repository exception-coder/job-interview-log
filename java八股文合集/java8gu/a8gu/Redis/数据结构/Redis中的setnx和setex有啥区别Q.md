...
- 不能设置过期时间SETEX  ,   SET with EXpiration，  设置值并指定过期时间
- 无条件进行设置，并带有过期时间​

#### SETNX 示例：- 如果 `holliskey` 不存在，就设置为 `"hello"`，**返回 **`**1**`- 如果 `holliskey` 已存在，不做修改，**返回 **`**0**`

#### ​ SETEX 示例：- **无论 **`**holliskey**`** 是否存在，都会设置为 **`**"hello"**`- 并设置过期时间为 60 秒（TTL）​
