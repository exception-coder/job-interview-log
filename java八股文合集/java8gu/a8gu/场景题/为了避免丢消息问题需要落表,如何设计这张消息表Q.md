...
- `state` 作为普通索引，用于高效扫表。这个也可以和`retry_count`、`next_retry_at`等字段一起建一个联合索引。- `message_key`  +`message_type` 作为唯一索引，防止重复插入​
...
### 状态信息| `state` | VARCHAR | 消息状态（如：待发送，已发送，已失败，已消费等） |
