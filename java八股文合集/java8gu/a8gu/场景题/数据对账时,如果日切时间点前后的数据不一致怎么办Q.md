...
- **结转账户数据**- 把当天的交易流水（借贷发生额）汇总到账户余额里。- 更新账户状态（比如存款计息、贷款结息）。- **生成对账/清算数据**- 输出日终对账文件（和支付清算系统、银联、同业机构做对账）。- 生成各种报表：分户账、总账、监管报表等。- **批量处理**- 跑批量任务（批量划扣、代发工资、结算资金）。- 清理过期数据，归档日志。- **切换会计日**- 从 T 日切换到 T+1 日，把系统日期推进。​

### 为什么会不一致

**最常见的原因就是业务发生时间不一致。**
- A系统使用业务发生时间（例如，用户发起支付的时间）。- B系统使用系统处理时间（例如，支付渠道处理成功的时间）。- 一笔在23:59:59发起的支付，可能在00:00:01才处理成功。A系统会把它算作前一天的数据，B系统则算作后一天。
