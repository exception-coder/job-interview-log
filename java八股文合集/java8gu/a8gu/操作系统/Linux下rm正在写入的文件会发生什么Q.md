...
- 文件名（Directory Entry）：用户看到的文件路径（如 `/data/file.txt`）。- 文件数据（Inode 和数据块）：实际存储文件内容的磁盘空间。
...
- 文件名是目录中的一个“链接（link）”，指向 inode。- 文件内容存在 inode 管理的 block 中。- `rm` 命令只是调用 `unlink()`，移除了一个目录项。
