...
│ 线程 A   │ │ 线程 B   │
│ 工作内存 │ │ 工作内存 │       JMM 三大问题：
│ x = 10   │ │ x = 10   │       ① 原子性 → synchronized / Atomic / CAS
│ ↓ +1     │ │ ↓ +2     │       ② 可见性 → volatile / synchronized / final
│ x = 11   │ │ x = 12   │       ③ 有序性 → volatile / synchronized / happens-before
└────┬─────┘ └──┬───────┘
     │ store    │ store         如果不加同步：
     │ write    │ write         两个线程都读到 x=10 各自 +1/+2 写回，
     ▼          ▼               最后 x 可能是 11 或 12，丢失更新
   ┌───────────────────┐
   │ 主内存 x = ?      │
   └───────────────────┘
```
...
```mermaid
flowchart LR
  U[无锁<br/>01] -->|第一次进入| B[偏向锁<br/>01 + thread ID<br/>JDK15 默认关]
  B -->|有竞争| L[轻量级锁<br/>00 指向 Lock Record<br/>线程栈帧 + CAS]
  L -->|自旋失败/竞争激烈| H[重量级锁<br/>10 指向 ObjectMonitor<br/>park/unpark 内核态]
...
```
对象头 Mark Word（64 位 JVM 中 64 位）：

无锁:        unused (25) | identity_hashCode (31) | unused (1) | age (4) | biased=0 (1) | lock=01
偏向锁:      ThreadID (54) | epoch (2) | unused (1) | age (4) | biased=1 (1) | lock=01
轻量级锁:    指向栈中 Lock Record 指针 (62)                                              | lock=00
重量级锁:    指向 ObjectMonitor 指针 (62)                                                | lock=10
GC 标记:                                                                                  | lock=11
...
│ owner（持锁线程）    │
│ recursions（重入数） │
│ entryList（阻塞队列）│ ← 抢锁失败的线程 park 在这
│ waitSet（等待集合）   │ ← 调 wait() 的线程在这
└─────────────────────┘
```
...
AQS 内部结构：
  ...
  Condition 单向链表（条件队列）：
  ┌────────┐ → ┌────────┐ → ┌────────┐
  │firstWaiter│ │ Node 4 │  │ Node 5 │
  └────────┘   └────────┘  └────────┘
  signalAll() → 把这些节点 transfer 到 CLH 队列尾，重新争锁
```

```mermaid
flowchart TD
  A[lock 调用] --> B[tryAcquire 尝试 CAS state]
  B -->|成功| OK[拿到锁返回]
  B -->|失败| C[addWaiter 入队]
  ...
  E -->|是| F[再次 tryAcquire]
  F -->|成功| G[出队 自己变 head]
  F -->|失败| H[shouldParkAfterFailedAcquire<br/>前驱状态置 SIGNAL]
  E -->|否| H
...
```mermaid
flowchart TD
  A[提交任务 execute task] --> B{工作线程数 < core?}
  B -->|是| C[创建核心线程执行]
  B -->|否| D{workQueue.offer 入队?}
  D -->|成功| E[等待被取出执行]
  D -->|失败 队列满| F{当前线程数 < max?}
  F -->|是| G[创建非核心线程执行]
  F -->|否| H[执行拒绝策略]
  
  H --> H1[AbortPolicy 抛异常]
  H --> H2[CallerRunsPolicy 提交线程自己跑]
  H --> H3[DiscardPolicy 静默丢弃]
  H --> H4[DiscardOldestPolicy 丢最老的再入队]
```

```
线程池调参经验：

CPU 密集型 → corePool = N+1（N=CPU核数）
IO 密集型  → corePool = 2N 或 N × (1 + IO等待/计算时间)
混合型     → 按业务实际压测

队列选择：
  ArrayBlockingQueue   有界 推荐
  LinkedBlockingQueue  默认无界 易 OOM ⚠
  SynchronousQueue     不存储 配合 max=Integer.MAX 是 newCachedThreadPool 坑
...
```
Thread (线程对象) ── ThreadLocalMap 字段（每个线程独有）
                        │
                        ▼
                 ┌─── Entry[] table ───┐
                 │                     │
                 │  Entry {            │
                 │    key:   ┌─────┐  │      WeakRef → ThreadLocal 对象
                 │           │ Weak │──┼───►(可能被 GC 回收 → key=null)
                 │           └─────┘   │
                 │    value: ┌─────┐  │      StrongRef → 用户的 value 对象
                 │           │Strong│──┼───►(只要 Thread 存活就一直占内存)
                 │           └─────┘   │
                 │  }                  │
                 └─────────────────────┘

线程池中泄漏链：
  Thread 复用 ─→ ThreadLocalMap ─→ Entry (key=null) ─→ value 永远不释放
                                                       ↑
                                                       ↑ 必须手动 remove() 才能清理！
...
stateDiagram-v2
  ...
  RUNNABLE --> TIMED_WAITING: sleep(t) / wait(t) / parkNanos
  TIMED_WAITING --> RUNNABLE: 超时 或 notify
...
| 操作 | 是否释放锁 | 进入状态 |
|---|---|---|
| Object.wait() | ✅ 释放 | WAITING |
| Thread.sleep() | ❌ 不释放 | TIMED_WAITING |
| LockSupport.park() | ❌ 不释放 | WAITING |
| Thread.yield() | 不释放，让出 CPU | RUNNABLE |
| Thread.join() | 不释放调用方的锁 | WAITING |
