...
```mermaid
flowchart TD
  S[① 实例化 createBeanInstance<br/>反射调构造器] --> P[② 属性填充 populateBean<br/>依赖注入发生在这]
  P --> AW[③ Aware 接口回调<br/>BeanNameAware/ApplicationContextAware]
  AW --> B1[④ BeanPostProcessor<br/>postProcessBeforeInitialization<br/>@PostConstruct 在这]
  B1 --> I1[⑤ InitializingBean.afterPropertiesSet]
  I1 --> I2[⑥ 自定义 init-method]
  I2 --> B2[⑦ BeanPostProcessor<br/>postProcessAfterInitialization<br/>✨ AOP 代理在这生成]
  B2 --> U[Bean 就绪 ✅ 使用]
  U --> D1[⑧ 容器关闭<br/>DisposableBean.destroy]
  D1 --> D2[⑨ 自定义 destroy-method<br/>@PreDestroy 同样在这]
  
  style B2 fill:#fcc
```

**3 个 init 钩子执行顺序**：`@PostConstruct` > `InitializingBean.afterPropertiesSet()` > 自定义 `init-method`
...
sequenceDiagram
  participant C as 容器
  participant S1 as singletonObjects 一级
  participant S2 as earlySingletonObjects 二级
  participant S3 as singletonFactories 三级
  participant A as Bean A
  participant B as Bean B
  
  C->>A: 实例化 A
  C->>S3: 放入 A 的 ObjectFactory
  C->>A: 填充属性，发现依赖 B
  C->>B: 实例化 B
  C->>S3: 放入 B 的 ObjectFactory
  C->>B: 填充属性，发现依赖 A
  C->>S1: 查 A → ❌
  C->>S2: 查 A → ❌
  C->>S3: 查 A → ✅ 拿 ObjectFactory
  Note over S3: getObject() 决定是否需要 AOP 代理<br/>返回早期 A (可能是代理)
  C->>S2: 早期 A 升到二级，三级移除
  C->>B: A 注入完成，B 初始化
  C->>S1: B 完成进入一级
  C->>A: A 拿到 B 完成属性填充
  ...
  Note over C,B: 解决不了的循环依赖:<br/>① 构造器循环 (实例化阶段就要对方)<br/>② prototype 作用域<br/>③ @Async + 循环依赖
```

**为什么需要三级而非二级**：核心是 **AOP 代理**。三级缓存存 `ObjectFactory`，**延迟决定**何时生成代理（不需要代理的 Bean 不强行提前代理）。
...
sequenceDiagram
  ...
  P->>A: @Around (前半段)
  P->>A: @Before
  P->>T: joinPoint.proceed()
  alt 正常返回
    T-->>P: result
    P->>A: @AfterReturning(result)
  else 抛异常
    T-->>P: exception
    P->>A: @AfterThrowing(ex)
  end
  P->>A: @After (finally 语义)
  P->>A: @Around (后半段)
  P-->>C: 最终返回
```
...
| 传播行为 | 外层有事务 | 外层无事务 | 内层回滚影响 |
|---|---|---|---|
| **REQUIRED**（默认） | 加入外层 | 新建 | 整个事务回滚 |
| **REQUIRES_NEW** | 挂起外层 + 新建 | 新建 | 只内层回滚 ✨ |
| **NESTED** | Savepoint 嵌套 | 新建 | 仅回滚到 Savepoint |
| **SUPPORTS** | 加入外层 | 非事务执行 | — |
| **NOT_SUPPORTED** | 挂起外层 + 非事务 | 非事务执行 | — |
| **NEVER** | 抛异常 | 非事务执行 | — |
| **MANDATORY** | 加入外层 | 抛异常 | 整个事务回滚 |

```mermaid
flowchart TD
  TX[事务调用] --> CHECK{当前有事务?}
  CHECK -->|有| WITH[REQUIRED 加入<br/>REQUIRES_NEW 挂起+新建<br/>NESTED Savepoint<br/>NEVER 抛异常]
  CHECK -->|无| WITHOUT[REQUIRED 新建<br/>SUPPORTS 非事务<br/>MANDATORY 抛异常]
```
