...
│  │  (数据/索引页 │  │ (非唯一二级 │  │ (redo 缓冲)│  │
│  │  LRU 冷热)   │  │ 索引合并写) │  └────────────┘  │
│  └──────────────┘  └─────────────┘  ┌────────────┐  │
│        ▲ 改良 LRU                   │Adaptive Hash│ │
│        │ new 63% / old 37%          └────────────┘  │
│  ─────────────────────────────────────────────────  │
│  后台线程                                            │
...
| 结构 | 1000 万数据需要的层数 | 范围查询 | 等值查询 |
|---|---|---|---|
| 二叉平衡树 | ~24 层 | 中序遍历 | O(log n) |
| 红黑树 | ~24 层 | 中序遍历 | O(log n) |
| B 树（fanout 100） | ~4 层但每层 IO 大 | 要回溯 | O(log_m n) |
| **B+ 树（fanout 1000）** | **3 层** | **沿叶子链表** | **3 次 IO** |
| Hash | 1 层 | ❌ 不支持 | O(1) |

```
B+ 树 3 层管亿级数据示意（非叶子节点 fanout=1000+）：

层 1 (Root)        ┌───────────┐
                   │ 1000 个 key│   16KB 页内有约 1000+ key
                   └─────┬─────┘   
                  ┌──────┼──────┐
                  ▼      ▼      ▼   (扇出 1000 次)
层 2 (非叶子)  ┌────┐ ┌────┐ ┌────┐
              │1000 │ │1000 │ │1000 │   1000 × 1000 = 100 万指针
              └─┬──┘ └─┬──┘ └─┬──┘
                ▼      ▼      ▼   (再扇出 1000 次)
层 3 (叶子)  [ data...data...data ]   100 万 × 1000 = 10 亿行
              │←─── 双向链表 ───→│

任何等值查询：3 次磁盘 IO（顶上 1-2 层往往已缓存在 Buffer Pool）
```
...
```
聚簇索引（主键 B+ 树）              二级索引（age 字段）
                                       
       [10|30|60]                       [25|35|50]
      /    |    \                      /    |    \
    叶子 叶子 叶子                    叶子 叶子 叶子
    │id=5│id=15│id=25│              │age=18,id=5│age=25,id=15│
    │完整行│完整行│完整行│            │  ↑回表查id=5            │
    └─────┴─────┴─────┘              └────────────────────────┘
       叶子存完整行数据                  叶子只存索引列+主键值
       
       回表过程：
       SELECT * FROM user WHERE age=18
       ① 二级索引找到 age=18 对应主键 id=5
       ② 拿 id=5 回到主键索引找完整行
       ③ 返回数据

       覆盖索引（无需回表）：
       SELECT id, age FROM user WHERE age=18
       → 二级索引叶子已含 (age, id)，直接返回  ✅ Using index
```
...
```
联合索引 INDEX(a, b, c) 命中规则：

查询条件                        命中情况
─────────────────────────────────────────────
WHERE a=1                       ✅ 用 a
WHERE a=1 AND b=2               ✅ 用 a, b
WHERE a=1 AND b=2 AND c=3       ✅ 用 a, b, c   (最佳)
WHERE b=2                       ❌ 不命中（跳过最左 a）
WHERE a=1 AND c=3               ⚠ 只用 a，c 走过滤
WHERE a=1 AND b>10 AND c=3      ⚠ 用 a, b，c 走过滤  (范围后截断)
WHERE a=1 ORDER BY b            ✅ a 索引 + b 排序避免 filesort

ICP 索引下推（5.6+ 默认开）：
  WHERE name LIKE 'Z%' AND age=20    INDEX(name, age)
  传统：① 用 name 找到候选行 → ② 回表 → ③ 在 server 层过滤 age
  ICP： ① 用 name 找候选 → ② 在引擎层直接用 age 过滤 → ③ 只回表通过的
  Extra: Using index condition  ← 这就是 ICP 生效
```
...
```mermaid
graph LR
  A[A 原子性] --> AU[undo log<br/>反向操作回滚]
  C[C 一致性] --> CU[业务约束 + PK/FK/CHECK<br/>+ AID 共同保证]
  I[I 隔离性] --> IL[行锁 + MVCC<br/>读写不冲突]
  D[D 持久性] --> DR[redo log + 双写<br/>WAL 崩溃恢复]
```

| 异常 | 隔离级别能否避免 | RU | RC | RR | Serializable |
|---|---|---|---|---|---|
| 脏读 | 读到未提交 | ❌ | ✅ | ✅ | ✅ |
| 不可重复读 | 同事务两读不同 | ❌ | ❌ | ✅ | ✅ |
| 幻读 | 范围查询行数变 | ❌ | ❌ | ⚠ 半解决 | ✅ |
...
```mermaid
flowchart TD
  S[读到一行记录 trx_id=X] --> Q1{X < min_trx_id?}
  Q1 -->|是 已提交且早于自己| OK[✅ 可见]
  Q1 -->|否| Q2{X ≥ max_trx_id?}
  Q2 -->|是 未来事务| NG[❌ 不可见 沿 undo 找前版]
  Q2 -->|否| Q3{X ∈ m_ids?}
  Q3 -->|是 活跃中| NG
  Q3 -->|否 已提交| OK
```
...
```
版本链（每行通过 roll_pointer 串联历史）：

最新行          旧版本 1           旧版本 2          旧版本 3
┌──────┐      ┌──────┐         ┌──────┐         ┌──────┐
│trx=50│←─────│trx=30│←────────│trx=10│←────────│ NULL │
│name=C│ rp   │name=B│ roll_ptr │name=A│         │      │
└──────┘      └──────┘         └──────┘         └──────┘

ReadView { m_ids=[40], min=40, max=51, creator=45 }
读这条记录：trx=50 不可见 → 沿 rp 到 trx=30 → 可见返回 B
```

```
RC vs RR：

RC（每次 select 新建 ReadView）：       RR（事务首次 select 创建后复用）：

t1: SELECT → ReadView_1 看到 v1         t1: SELECT → ReadView_1 看到 v1
t2: 别人改了并提交                       t2: 别人改了并提交
t3: SELECT → ReadView_2 看到 v2 ✱       t3: SELECT → 还用 ReadView_1 看到 v1 ✓
    (不可重复读)                            (可重复读)
```
...
Gap Lock 锁间隙（开区间）：
                       (10────15)     只锁 (10,15) 之间，不含 10 和 15
                       
Next-Key Lock 左开右闭（默认）：
                       (10────15]     锁 (10,15] 包含 15

例：UPDATE t SET ... WHERE age=15 (假设 age 有索引，RR 级别)
  默认加 Next-Key Lock: (10, 15] 和 (15, 20)  → 防止幻读

插入意向锁：多个事务想在同一间隙不同位置插入不冲突
```

| 锁 | 级别 | 互斥规则 | 何时加 |
|---|---|---|---|
| S | 行 | 与 X 冲突 | SELECT ... LOCK IN SHARE MODE |
| X | 行 | 与所有冲突 | UPDATE / DELETE / SELECT ... FOR UPDATE |
| IS | 表 | 与表 X 冲突 | 加 S 行锁前先加 |
| IX | 表 | 与表 S, X 冲突 | 加 X 行锁前先加 |
| Record | 行 | 锁单行 | 唯一索引等值命中 |
| Gap | 间隙 | 防插入 | RR 范围查询 / 唯一索引等值不命中 |
| Next-Key | 行+间隙 | 防幻读 | RR 默认（5.7+ 大多场景） |
| MDL | 表元数据 | DML/DDL 互斥 | DML 持 MDL 读、DDL 持 MDL 写 |
...
sequenceDiagram
  ...
  C->>S: UPDATE t SET ...
  S->>I: 执行 update
  I->>I: 改 Buffer Pool 数据页（脏页）
  I->>I: 写 undo log（用于回滚 + MVCC）
  I->>I: 写 redo log (prepare 状态)
  S->>S: 写 binlog
  I->>I: 写 redo log (commit 状态)
  S-->>C: OK

  Note over I,S: 崩溃恢复:<br/>① redo prepare + binlog 完整 → 提交<br/>② redo prepare 但 binlog 缺失 → 回滚
```

| 日志 | 层 | 类型 | 写入方式 | 用途 | 关键参数 |
|---|---|---|---|---|---|
| redo log | InnoDB | 物理日志 | 循环写 | 持久性 / 崩溃恢复 | innodb_flush_log_at_trx_commit |
| undo log | InnoDB | 逻辑日志 | 段管理 | 原子性 / MVCC | innodb_undo_log_truncate |
| binlog | Server | 逻辑日志 | 追加写 | 主从复制 / 时点恢复 | sync_binlog / binlog_format |
...
sequenceDiagram
  ...
  participant IT as IO thread
  participant RL as relay log
  participant ST as SQL thread
  participant S as Slave
  App->>M: 写操作（事务提交）
  M->>M: 写 binlog
  DT->>IT: 推送 binlog event
  IT->>RL: 写入 relay log
  ST->>RL: 读取 event
  ST->>S: 重放 SQL
  
  Note over IT,ST: 延迟来源:<br/>① SQL thread 单线程重放(老版本)<br/>② 大事务<br/>③ 网络/硬件<br/>5.7+ 组提交并行 / 8.0 WriteSet 并行
```
