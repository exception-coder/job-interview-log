...
```
正排索引 (传统数据库)：           倒排索引 (ES/Lucene)：

  doc_id → 内容                    词项 → 包含该词的 doc_id 列表
  ─────────────                    ─────────────────────────────
  doc1: Java 是面向对象语言         "Java"  → [doc1, doc2, doc5]
  doc2: Java 性能优化               "性能"  → [doc2, doc7]
  doc5: Java 并发编程               "并发"  → [doc3, doc5]
  doc7: 性能调优实战                "对象"  → [doc1]
  
  搜"Java" 要全表扫所有 doc        搜"Java" 直接拿到 [doc1, doc2, doc5]
  适合: 按主键定位/全字段读         适合: 全文检索/词项查询
```

```mermaid
flowchart LR
  W[写请求] --> M[内存 Buffer + Translog 顺序写]
  M -->|每 1s refresh| O[OS Cache Segment<br/>可搜索 ✨ NRT 近实时]
  O -->|每 30min 或<br/>translog 满 512MB| D[flush 写盘<br/>清空 translog]
  
  O -.后台 Merge.-> O2[Segment 合并<br/>清理已删]
  
  style O fill:#fcc
```

**Segment 不可变**：删除 = .del 文件标记 + 后台 Merge 真正物理删；更新 = 旧文档标删 + 新文档。
...
```mermaid
flowchart TD
  C[Client 数据] --> SOC[Socket]
  SOC --> CH[Channel]
  CH --> EL[EventLoop<br/>绑定一个线程]
  EL --> PIPE[ChannelPipeline]
  
  PIPE --> H1[Decoder 解码<br/>LengthFieldBasedFrameDecoder]
  H1 --> H2[业务 Handler<br/>BusinessLogic]
  H2 --> H3[Encoder 编码<br/>转字节]
  H3 --> CH2[Channel 写回]
...
发送端: msg1 = "Hello"   msg2 = "World"

接收端可能收到:
  情况 1 (正常):          [Hello] [World]
  情况 2 (粘包):          [HelloWorld]
  情况 3 (拆包):          [Hel] [loWor] [ld]
  情况 4 (混合):          [Hel] [loWorl] [d]

解决 (应用层协议)：
  ① 定长          → FixedLengthFrameDecoder(5)
  ② 分隔符        → DelimiterBasedFrameDecoder("\r\n\r\n")  (HTTP 用)
  ③ 长度前缀 ⭐推荐 → LengthFieldBasedFrameDecoder
                    [4字节长度][N字节数据]
  ④ TLV           → Type-Length-Value 自定义协议
```
