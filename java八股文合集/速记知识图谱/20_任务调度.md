...
```mermaid
flowchart TD
  ADMIN[调度中心<br/>Admin Console] -->|cron 触发| ROUTE{路由策略}
  ROUTE -->|FIRST/LAST| E1[执行器 1]
  ROUTE -->|ROUND/RANDOM| E2[执行器 2]
  ROUTE -->|CONSISTENT_HASH| E3[执行器 3]
  ROUTE -->|SHARDING_BROADCAST| ALL[全部执行器<br/>每个拿分片号]
  ...
  style ADMIN fill:#fcc
  style ROUTE fill:#cff
```

```
分片广播示例 (海量数据任务)：

总数据 100 万条 + 10 个执行器实例
                         
ADMIN 调度时同时通知 10 个执行器:
  执行器 1: 分片号=0, 总分片=10  → 处理 user_id % 10 == 0 的数据
  执行器 2: 分片号=1, 总分片=10  → 处理 user_id % 10 == 1 的数据
  ...
  执行器 10: 分片号=9, 总分片=10 → 处理 user_id % 10 == 9 的数据

  → 100 万数据 10 倍并行, 10 分钟变 1 分钟
```
